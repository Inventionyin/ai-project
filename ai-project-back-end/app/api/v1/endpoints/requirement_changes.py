...
from app.schemas.knowledge import RecommendationEvaluateRequest, RecommendationEvaluateResponse
...
from app.services.knowledge import evaluate_recommendations
from app.services.requirement_change import (
    ...
    get_requirement_regression_set_by_change_set,
...
@router.get(
    "/projects/{projectId}/requirements/change-sets/{changeSetId}/knowledge-recommendations",
    response_model=ApiResponse[RecommendationEvaluateResponse],
)
async def get_change_set_knowledge_recommendations(
    projectId: uuid.UUID,
    changeSetId: uuid.UUID,
    ...
    try:
        data = await evaluate_recommendations(
            db,
            user=user,
            project_id=projectId,
            payload=RecommendationEvaluateRequest(
                targetType="CHANGE_SET",
                targetId=str(changeSetId),
                topK=topK,
            ),
        )
        await db.commit()
...
@router.get("/projects/{projectId}/requirements/change-sets/{changeSetId}/regression-set", response_model=ApiResponse[RequirementRegressionSetDetail | None])
async def get_regression_set_by_change_set(
    projectId: uuid.UUID,
    changeSetId: uuid.UUID,
    db: AsyncSession = Depends(get_db),
    user: CurrentUser = Depends(get_current_user),
    request_id: str = Depends(get_request_id),
) -> ApiResponse[RequirementRegressionSetDetail | None]:
    data = await get_requirement_regression_set_by_change_set(db, user=user, project_id=projectId, change_set_id=changeSetId)
    return ApiResponse(data=data, requestId=request_id)
