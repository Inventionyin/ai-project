...
@router.get("/diagnostics", response_model=ApiResponse[NotificationDiagnosticsData])
async def get_project_notification_diagnostics(
    projectId: uuid.UUID,
    db: AsyncSession = Depends(get_db),
    user: CurrentUser = Depends(get_current_user),
    request_id: str = Depends(get_request_id),
) -> ApiResponse[NotificationDiagnosticsData]:
    data = await get_notification_diagnostics(db, user=user, project_id=projectId)
    return ApiResponse(data=data, requestId=request_id)
