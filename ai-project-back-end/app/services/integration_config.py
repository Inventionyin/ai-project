...
_NOTIFICATION_SUPPORTED_PROVIDERS = frozenset({"WEBHOOK", "EMAIL", "IM", "DINGTALK"})
_FAILED_DELIVERY_STATUSES = frozenset({"FAILED"})
...
def _safe_rule_events(rule: dict[str, object]) -> list[str]:
    events = rule.get("events")
    if not isinstance(events, list):
        return []
    return [str(item).strip().upper() for item in events if str(item).strip()]


def _check_missing_or_invalid_target(*, channel: str, target: str) -> bool:
    normalized_channel = str(channel or "").strip().upper()
    normalized_target = str(target or "").strip()
    if not normalized_target:
        return True
    if normalized_channel == "WEBHOOK":
        return not (normalized_target.startswith("http://") or normalized_target.startswith("https://"))
    if normalized_channel == "EMAIL":
        return "@" not in normalized_target
    if normalized_channel == "IM":
        return len(normalized_target) < 2
    return True


async def _check_template_reference_exists(
    db: AsyncSession,
    *,
    user: CurrentUser,
    project_id: uuid.UUID,
    ...
    *,
    ...
    outbox_rows = (
        await db.execute(
            ...
            .order_by(desc(NotificationOutbox.updated_at), desc(NotificationOutbox.id))
        )
    ).scalars().all()

    checks: list[NotificationDiagnosticsCheck] = []
    recent_failures: list[NotificationDiagnosticsFailure] = []
    ...
    if not notifications:
        checks.append(
            NotificationDiagnosticsCheck(
                ...
                detail="This project has no notification integration rules.",
                recommendation="Add at least one notification rule and enable required events.",
                notificationId=None,
            )
        )

    provider_counts: dict[str, int] = {}
    provider_issue_reasons: dict[str, list[str]] = {}
    provider_config_fields: dict[str, set[str]] = {}

    def mark_provider_issue(provider_name: str, reason: str) -> None:
        if not provider_name:
            provider_name = "UNKNOWN"
        reasons = provider_issue_reasons.setdefault(provider_name, [])
        if reason not in reasons:
            reasons.append(reason)

    def remember_provider_fields(provider_name: str, fields: list[str]) -> None:
        if not provider_name:
            provider_name = "UNKNOWN"
        bucket = provider_config_fields.setdefault(provider_name, set())
        for field in fields:
            if field:
                bucket.add(field)

    external_provider_requirements: dict[str, list[tuple[str, tuple[str, ...]]]] = {
        "WEBHOOK": [
            ("target", ("target",)),
        ...
        "IM": [
            ("webhookUrl", ("webhookUrl", "botWebhook", "url")),
        ],
        "DINGTALK": [
            ("webhookUrl", ("target", "webhookUrl", "botWebhook", "url")),
            ("keyword", ("keyword",)),
        ],
        "JIRA": [
            ("baseUrl", ("baseUrl", "url")),
        ...
        "GITHUB": [
            ("baseUrl", ("baseUrl", "apiUrl", "url")),
            ("token", ("token", "accessToken")),
            ("repository", ("repository", "repo")),
            ("callbackSecret", ("callbackSecret", "webhookSecret", "secret")),
        ],
        "JENKINS": [
            ("baseUrl", ("baseUrl", "url")),
            ("token", ("token", "apiToken")),
    ...
    def resolve_rule_value(rule: dict[str, object], target: str, aliases: tuple[str, ...]) -> str:
        for key in aliases:
            raw = target if key == "target" else rule.get(key)
    ...
    for notification in notifications:
        ...
        channel = str(notification.channel or "").strip().upper()
        rule = dict(notification.rule_json or {})
        enabled = bool(notification.enabled)
        provider = str(rule.get("provider") or channel).strip().upper()

        provider_counts[provider] = int(provider_counts.get(provider, 0)) + 1
        missing_external_fields: list[str] = []
        configured_external_fields: list[str] = []
        for field_name, aliases in external_provider_requirements.get(provider, []):
            if resolve_rule_value(rule, str(notification.target or ""), aliases):
                configured_external_fields.append(field_name)
            else:
                missing_external_fields.append(field_name)
        remember_provider_fields(provider, configured_external_fields)
        if missing_external_fields:
            checks.append(
                NotificationDiagnosticsCheck(
                    id=f"{notification_id}:provider_config_missing",
                    level="BLOCKER" if provider not in {"WEBHOOK"} else "WARNING",
                    scope="provider",
                    title="External provider configuration incomplete",
                    detail=f"Provider {provider} is missing required fields: {', '.join(missing_external_fields)}.",
                    recommendation="Complete provider credentials/callback fields before real external-system joint testing.",
                    notificationId=notification_id,
                )
            )
            mark_provider_issue(provider, "CONFIG_MISSING:" + ",".join(missing_external_fields))

        if not enabled:
            checks.append(
                NotificationDiagnosticsCheck(
                    id=f"{notification_id}:disabled",
                    level="WARNING",
                    scope="rule",
                    title="Rule is disabled",
                    detail=f"Notification rule {notification_id} is disabled.",
                    recommendation="Enable this rule if notifications are expected.",
                    notificationId=notification_id,
                )
            )
            mark_provider_issue(provider, "RULE_DISABLED")

        if _check_missing_or_invalid_target(channel=channel, target=str(notification.target or "")):
            checks.append(
                NotificationDiagnosticsCheck(
                    id=f"{notification_id}:target_invalid",
                    level="BLOCKER",
                    scope="rule",
                    title="Missing or invalid target",
                    detail=f"Notification target is invalid for channel {channel or 'UNKNOWN'}.",
                    recommendation="Provide a valid destination target for this channel.",
                    notificationId=notification_id,
                )
            )
            mark_provider_issue(provider, "TARGET_INVALID")

        if provider not in _NOTIFICATION_SUPPORTED_PROVIDERS:
            checks.append(
                NotificationDiagnosticsCheck(
                    id=f"{notification_id}:provider_unsupported",
                    level="BLOCKER",
                    scope="provider",
                    title="Unsupported provider",
                    detail=f"Provider {provider or 'UNKNOWN'} is not supported.",
                    recommendation="Use WEBHOOK, EMAIL, IM, or DINGTALK as provider.",
                    notificationId=notification_id,
                )
            )
            mark_provider_issue(provider, "UNSUPPORTED_PROVIDER")

        events = _safe_rule_events(rule)
        if len(events) == 0:
            checks.append(
                NotificationDiagnosticsCheck(
                    id=f"{notification_id}:events_missing",
                    level="BLOCKER",
                    scope="rule",
                    title="No events configured",
                    detail=f"Notification rule {notification_id} has no events.",
                    recommendation="Configure at least one triggering event.",
                    notificationId=notification_id,
                )
            )
            mark_provider_issue(provider, "EVENTS_MISSING")

        has_template_name = bool(str(rule.get("templateName") or "").strip())
        has_template_version = bool(str(rule.get("templateVersion") or "").strip())
        if has_template_name or has_template_version:
            exists = await _check_template_reference_exists(
                db,
                user=user,
                project_id=project.id,
                channel=channel,
            ...
            if not exists:
                checks.append(
                    NotificationDiagnosticsCheck(
                        id=f"{notification_id}:template_missing",
                        level="BLOCKER",
                        scope="template",
                        title="Referenced prompt template not found",
                        detail=f"Template reference in notification rule {notification_id} cannot be resolved.",
                        recommendation="Create the referenced prompt template or fix templateName/templateVersion.",
                        notificationId=notification_id,
                    )
                )
                mark_provider_issue(provider, "TEMPLATE_MISSING")

        if channel == "WEBHOOK" and not str(rule.get("secret") or "").strip():
            checks.append(
                NotificationDiagnosticsCheck(
                    id=f"{notification_id}:webhook_secret_missing",
                    level="WARNING",
                    scope="security",
                    title="Webhook secret is missing",
                    detail=f"Webhook notification rule {notification_id} has no secret.",
                    recommendation="Set rule.secret to protect webhook requests.",
                    notificationId=notification_id,
                )
            )
            mark_provider_issue(provider, "WEBHOOK_SECRET_MISSING")

    for row in outbox_rows:
        status = str(row.status or "").upper()
        if status in _FAILED_DELIVERY_STATUSES:
    ...
    if recent_failures:
        failed_providers = sorted({str(row.provider or "").strip().upper() or "UNKNOWN" for row in outbox_rows if str(row.status or "").upper() in _FAILED_DELIVERY_STATUSES})
        for provider in failed_providers:
            mark_provider_issue(provider, "RECENT_FAILED_DELIVERY")
        checks.append(
            NotificationDiagnosticsCheck(
                ...
                detail=f"{len(recent_failures)} failed delivery records exist.",
                recommendation="Inspect failure details and retry after fixing configuration issues.",
                notificationId=None,
            )
        )

    provider_readiness: list[NotificationDiagnosticsProviderReadiness] = []
    for provider, count in sorted(provider_counts.items()):
        reasons = provider_issue_reasons.get(provider, [])
        ready = provider in _NOTIFICATION_SUPPORTED_PROVIDERS and not reasons
        if provider not in _NOTIFICATION_SUPPORTED_PROVIDERS:
        ...
        provider_readiness.append(
            NotificationDiagnosticsProviderReadiness(
                ...
                configured=provider in _NOTIFICATION_SUPPORTED_PROVIDERS and not any(r.startswith("CONFIG_MISSING") for r in reasons),
                missingFields=sorted(
                    {
                        field
                        for reason_item in reasons
                        if reason_item.startswith("CONFIG_MISSING:")
                        for field in reason_item.split(":", 1)[1].split(",")
                        if field
                    }
                ),
                diagnostics={
                    "configuredFields": sorted(provider_config_fields.get(provider, set())),
                    "realConnectivityVerified": False,
                    "realConnectivityNote": "Credentials and target callbacks must be verified in the deployment environment.",
                },
            )
        )

    blocking = sum(1 for item in checks if item.level == "BLOCKER")
    warnings = sum(1 for item in checks if item.level == "WARNING")
    ready = sum(1 for item in checks if item.level == "READY")
    failed_deliveries = len(recent_failures)
    if blocking > 0:
        status = "BLOCKED"
    elif warnings > 0 or failed_deliveries > 0:
        status = "WARN"
    else:
        status = "READY"
    ...
    return NotificationDiagnosticsData(
        ...
        checks=checks,
        recentFailures=recent_failures[:20],
