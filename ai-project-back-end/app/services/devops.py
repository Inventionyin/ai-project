...
from app.schemas.devops import DevOpsPipelineDetail, DevOpsRunDetail
...
def _request_auth_from_config(config: dict) -> tuple[str, str] | None:
    ...
    api_token = str(config.get("api_token") or config.get("jenkins_api_token") or "").strip()
...
def _trigger_github_actions(
    *,
    ...
    gh_token = str(config.get("github_token") or "").strip()
    if not gh_token:
    ...
    resp = http_requests.post(
        ...
        json={"ref": branch or "main", "inputs": inputs},
        timeout=10,
    )
    if resp.status_code in (200, 201, 204):
        return True, None, None
    return False, f"GitHub API error: {resp.status_code} {resp.text[:500]}", None
...
def _trigger_jenkins(
    *,
    ...
    config = pipeline.config_json or {}
    base_url = str(config.get("jenkins_url") or config.get("base_url") or "").strip().rstrip("/")
    job_name = str(config.get("job_name") or pipeline.workflow_file or pipeline.repo_full_name or "").strip()
    if not base_url:
    ...
    trigger_token = str(config.get("trigger_token") or "").strip()
    if trigger_token:
    ...
    return False, f"Jenkins API error: {resp.status_code} {resp.text[:500]}", None
...
async def trigger_pipeline(
    ...
    await db.flush()
    if pipeline.provider == "github_actions" and pipeline.repo_full_name and pipeline.workflow_file:
        try:
