        ...
        <table class="w-full min-w-[860px] border-collapse">
          ...
          <tbody>
            <tr v-for="doc in docs" :key="doc.id" class="cursor-pointer border-t border-black/10 hover:bg-[#F8FAFC]" @click="openDetail(doc)">
              <td class="px-4 py-3 text-[13px] text-[#0A0A0A]">{{ doc.title }}</td>
              <td class="px-4 py-3">
                <span class="rounded-full px-2 py-[2px] text-[11px] font-medium" :class="statusClass(doc.status)">{{ statusText(doc.status) }}</span>
              </td>
              <td class="px-4 py-3 text-[12px] text-[#4A5565]">{{ sourceTypeText(doc.sourceType) }}</td>
              <td class="px-4 py-3 text-[12px] text-[#4A5565]">{{ doc.tags?.join(', ') || '-' }}</td>
