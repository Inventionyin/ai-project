    ...
    <div class="rounded-[14px] border border-black/10 bg-white p-6">
      ...
      <div v-if="showCreate" class="mb-4 p-4 border border-black/10 rounded bg-gray-50">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <input v-model="form.workflowFile" placeholder="工作流文件名 (ci.yml)" class="text-[12px] border rounded px-2 py-1" />
        </div>
...
const form = ref({ name: '', provider: 'github_actions', repoFullName: '', workflowFile: '' })
...
async function create() {
  ...
  await createPipeline(projectId, form.value)
  showCreate.value = false
  form.value = { name: '', provider: 'github_actions', repoFullName: '', workflowFile: '' }
  await load()
}
